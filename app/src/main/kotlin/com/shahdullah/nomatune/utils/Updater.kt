...
object Updater {
    ...
    private suspend fun fetchReleasesNetwork(
        ...
        val response: HttpResponse =
            client.get("https://api.github.com/repos/Shahdullah/NomaTune/releases?per_page=$perPage") {
                headers {
    ...
    suspend fun getCommitHistory(count: Int = 20, branch: String = "dev"): Result<List<GitCommit>> =
        runCatching {
            ...
            val response =
                client.get("https://api.github.com/repos/Shahdullah/NomaTune/commits?sha=$branch&per_page=$count")
                    .bodyAsText()
    ...
    fun getLatestDownloadUrl(): String {
        ...
        if (tag != null) {
            return "https://github.com/Shahdullah/NomaTune/releases/download/$tag/app-$distributionArtifactPrefix${BuildConfig.DEVICE}-${BuildConfig.ARCHITECTURE}-release.apk"
        }
    ...
    fun getLatestNightlyDownloadUrl(): String {
        ...
        return "https://nightly.link/Shahdullah/NomaTune/workflows/build/dev/app-$distributionArtifactPrefix${BuildConfig.DEVICE}-${BuildConfig.ARCHITECTURE}-release"
    }
